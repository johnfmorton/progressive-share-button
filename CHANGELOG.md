...
- Guard `customElements.define()` with `customElements.get()` to prevent `NotSupportedError` when the module is loaded more than once (bundler duplication, HMR, multiple script tags, etc.)
